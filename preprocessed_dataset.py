        ...
        for i, main_dir in enumerate(os.listdir(DATASET_PATH)):
            ...
            for file_name in tqdm(os.listdir(sub_dir), desc=f"Step {i+1}/27, {main_dir}"):
                ...
                for hand_landmarks in results.multi_hand_landmarks:
                    for i in range(0, 21):
                        current_file.append(np.asarray([current_data.append(np.asarray([hand_landmarks.landmark[i].x, hand_landmarks.landmark[i].y, hand_landmarks.landmark[i].z], dtype=np.float]))
))
